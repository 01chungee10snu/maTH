...
function drawQuiz() {
    ...
    if (buttonW < minButtonSize && numOptions > 3) {
    ...
    for (let i = 0; i < options.length; i++) {
        ...
        const oy = baseY + row * (buttonH + buttonGap);
...
function drawCollection() {
    ...
    const gridY = Math.round(96 * SCALE);
    ...
    TINIPINGS.forEach((t, idx) => {
        ...
        CTX.save();
        roundRect(CTX, x, y, w, h, Math.round(12 * SCALE));
        CTX.fillStyle = '#ffffff';
        CTX.fill();
        ...
        const centerX = x + w / 2;
        const centerY = y + Math.round(45 * SCALE);
        const size = Math.max(48, Math.round(58 * SCALE));

        if (STATE.caughtIds.includes(t.id) && t.imageObj) {
            CTX.drawImage(t.imageObj, centerX - size / 2, centerY - size / 2, size, size);
            CTX.fillStyle = '#374151';
            CTX.font = `${Math.round(14 * SCALE)}px Jua, sans-serif`;
            CTX.fillText(t.name, centerX, y + h - Math.round(20 * SCALE));
        } else {
            CTX.fillStyle = '#9ca3af';
            CTX.font = `bold ${Math.round(28 * SCALE)}px Jua, sans-serif`;
            CTX.fillText('?', centerX, centerY + Math.round(10 * SCALE));
            CTX.fillStyle = '#9ca3af';
            CTX.font = `${Math.round(14 * SCALE)}px Jua, sans-serif`;
            CTX.fillText('???', centerX, y + h - Math.round(20 * SCALE));
        }
    });
}
