...
운영 초기는 `b`를 수동으로 넣고, 응답 로그가 충분히 쌓이면 정답률과 힌트 사용량을 기준으로 재보정한다. 초기 seed 은행은 최소 30문항을 유지하고, 핵심 skill은 각각 최소 2문항 이상을 둔다. 새 단원을 추가할 때는 먼저 10문항 단위로 seed를 넣고, 실사용 로그가 쌓이면 난이도 `b`를 재추정한다.
